...
import {
  ...
  Sparkles, ChevronRight, ChevronLeft, BookOpen, X, Library, Briefcase, Database, CreditCard
} from 'lucide-react';
...
const AdminSidebar = ({ isDesktopOpen, isMobileOpen, toggleDesktopSidebar, closeMobileMenu, currentView, onNavigate, onLogout }: AdminSidebarProps) => {

  const menuItems = [
    { id: 'overview', name: 'لوحة التحكم', icon: <LayoutDashboard size={20} /> },
    { id: 'ai-creator', name: 'منشئ الدورات (AI)', icon: <Sparkles size={20} />, highlight: true },
    { id: 'ai-drafts', name: 'مسودات AI', icon: <Database size={20} /> },
    { id: 'courses', name: 'إدارة الدورات', icon: <BookOpen size={20} /> },
    { id: 'library', name: 'إدارة المكتبة', icon: <Library size={20} /> },
    { id: 'consultations', name: 'إدارة الاستشارات', icon: <Briefcase size={20} /> },
    { id: 'users', name: 'المستخدمين', icon: <Users size={20} /> },
    { id: 'withdrawals', name: 'طلبات السحب', icon: <CreditCard size={20} /> },
    { id: 'finance', name: 'المدفوعات', icon: <Wallet size={20} /> },
  ...
  return (
    <aside
      className={`
        fixed top-0 right-0 h-full bg-[#0f172a]/95 backdrop-blur-xl border-l border-white/5 z-40 shadow-2xl flex flex-col justify-between transition-all duration-300
    ...
    >
      ...
      <div className="h-28 flex items-center justify-center border-b border-white/5 relative shrink-0">
        <div className={`flex flex-col items-center transition-all ${!isDesktopOpen && 'lg:scale-75'}`}>
          ...
          />
          {(isDesktopOpen || isMobileOpen) && <span className="text-[10px] tracking-widest text-brand-gold uppercase font-bold text-center">لوحة الإدارة</span>}
        </div>
